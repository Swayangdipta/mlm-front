...
const TeamBusiness = () => {
  ...
  useEffect(() => {
    (async () => {
        ...
        const promiseArray = downline.map(async (item) => {
            return getTotalTeamBusiness(item._id)
        })
        ...
        if(failedRequests.length > 0){
            return toast.error('Failed to get downline data')
        }

        const teamBusinesses = results.map(result => {
            return {totalBusiness: result.data.totalBusiness, topName: result.data.topName, topCode: result.data.topCode}
        })

        setBusinesses(teamBusinesses)
    })()
}, [downline]);
    ...
    <div className="min-h-screen bg-blue-50">
      ...
      <main className="pt-36 w-full min-h-screen bg-[url(https://ehsslibrary2.wordpress.com/wp-content/uploads/2014/04/blank-google-plus-background-navy-blue.jpg)]">
        <div className="w-[90%] min-h-[400px] h-max p-4 mx-auto rounded border-amber-500 border bg-[#00000080] shadow-inner">
          <h2 className="text-zinc-100">My Team Business</h2>
            ...
            <table className="table-auto mt-4 w-full">
              <thead className="w-full bg-zinc-300 border">
                <tr>
                  ...
                  <th className="border border-zinc-400 text-sm md:text-base">Team Business</th>
                </tr>
              </thead>
                ...
                {businesses.length > 0 ? (
                  businesses.map((item, index) => (
                    <tr key={item._id} className="border">
                      ...
                      <td className="border border-zinc-400">{item.topCode}</td>
                      <td className="border border-zinc-400">
                        {item.topName}
                      </td>
                      <td className="border border-zinc-400">
